...
class RemoteFeedLoaderTests: XCTestCase {
    ...
    private func expect(
        ...
        sut.load { recievedResult in
            ...
            case (let .success(recievedItems), let .success(expectedItems)):
                XCTAssertEqual(recievedItems, expectedItems, file: file, line: line)
            case (let .failure(recievedError as RemoteFeedLoader.Error), let .failure(expectedError as RemoteFeedLoader.Error)):
                XCTAssertEqual(recievedError, expectedError, file: file, line: line)
            default:
                XCTFail("Expected result: \(expectedResult), but got \(recievedResult) instead")
            }
